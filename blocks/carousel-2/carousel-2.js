...
function createSlide(row, slideIndex, carouselId) {
  ...
  slide.setAttribute('id', `carousel-2-${carouselId}-slide-${slideIndex}`);
  slide.classList.add('carousel-2-slide');

  row.querySelectorAll(':scope > div').forEach((column, colIdx) => {
    column.classList.add(`carousel-2-slide-${colIdx === 0 ? 'image' : 'content'}`);
    slide.append(column);
  });
...
export default async function decorate(block) {
  carouselId += 1;
  block.setAttribute('id', `carousel-2-${carouselId}`);
  ...
  if (!isSingleSlide) {
    ...
    slideNavButtons.classList.add('carousel-2-navigation-buttons');
